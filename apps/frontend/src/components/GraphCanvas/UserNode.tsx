...
function UserNodeComponent(props: NodeProps): JSX.Element {
  ...
  return (
    <div
      className={`rounded-lg border-l-4 border-l-node-user bg-panel pl-2 pr-3 py-2 text-left shadow-panel transition-opacity ${borderClass} ${privilegeClass} ${opacityClass}`}
      title={data.title}
    >
      <Handle
      ...
      <div className="flex items-start gap-2">
        <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-node-user" aria-hidden />
        <div className="min-w-0 flex-1">
          <p className="truncate text-label font-semibold uppercase tracking-wide text-node-user">
            User
          </p>
